    ...
    return (
        <div className="w-full max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden space-y-6">
            ...
            <div className="p-6 space-y-6">
                {items.length === 0 ? (
                    ...
                    <div key={index} className="relative p-4 rounded-xl border border-slate-100 bg-slate-50/30 group">
                        <button onClick={() => removeItem(index)} className="absolute -top-2 -right-2 p-1 cursor-pointer bg-white border border-slate-200 text-slate-400 rounded-full hover:text-red-500 hover:border-red-200 shadow-sm opacity-0 group-hover:opacity-100 transition-all">
                            <X size={14} />
                        </button>
            ...
            <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex gap-3">
                <button onClick={handleSave} disabled={items.length === 0} className="flex-1 flex items-center justify-center gap-2 px-6 py-2.5 bg-[#010221] text-white rounded-lg text-sm font-bold shadow-lg shadow-blue-100 hover:bg-[#010221]/90 cursor-pointer transition-all disabled:opacity-50">
                    <Save size={18} /> Save All ({items.length})
                </button>
                <button onClick={onCancel} className="px-6 py-2.5 border border-slate-200 text-slate-600 rounded-lg text-sm font-bold hover:bg-white transition-all cursor-pointer">Cancel</button>
            </div>
        </div>
    );
}